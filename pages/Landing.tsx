    ...
    <div className="min-h-screen flex flex-col pt-20 cyber-grid">
      ...
      <section className="relative px-6 py-16 md:py-24 max-w-7xl mx-auto w-full grid lg:grid-cols-2 items-center gap-12">
        <div className="flex flex-col gap-6">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-surface-dark border border-dark w-fit">
          ...
          <p className="text-slate-400 text-lg md:text-xl max-w-xl font-body leading-relaxed">
            Akses premium Top Up untuk semua Game favoritmu. Aman, cepat, dan terpercaya dengan teknologi enkripsi terbaru.
          </p>
          <div className="flex gap-4 pt-4">
            <button
              onClick={() => document.getElementById('games-section')?.scrollIntoView({ behavior: 'smooth' })}
              className="bg-primary text-background-dark px-8 py-4 rounded-xl font-bold tracking-wide shadow-neon hover:scale-105 transition-all flex items-center gap-2">
              <span className="material-symbols-outlined">bolt</span> MULAI BELANJA
            </button>
            <button
              onClick={handleShowPromo}
              className="bg-surface-dark border border-dark px-8 py-4 rounded-xl font-bold hover:bg-white/5 transition-all flex items-center gap-2"
            >
        ...
        <div className="hidden lg:block relative group">
          ...
          <div className="relative glass-panel rounded-2xl p-4 shadow-2xl overflow-hidden aspect-[4/3] rotate-1 group-hover:rotate-0 transition-transform duration-700">
            <img
              src={activeBanner ? activeBanner.imageUrl : "https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&q=80&w=800"}
              className="w-full h-full object-cover rounded-xl"
              alt={activeBanner ? activeBanner.title : "Gaming"}
            />
            <div className="absolute top-6 right-6 px-4 py-2 bg-secondary/80 backdrop-blur rounded-full border border-white/20 text-xs font-bold shadow-2xl">
              {activeBanner ? activeBanner.title : '24/7 SUPPORT READY'}
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Filter & Search */}
      <div className="sticky top-20 z-40 py-6 px-6 glass-panel border-y border-dark">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-6">
          <div className="flex-1 relative w-full">
            ...
            <input
              ...
              className="w-full bg-surface-accent border border-dark rounded-2xl pl-12 pr-4 py-4 text-main focus:outline-none focus:border-primary transition-all font-body text-lg shadow-inner"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
            />
          </div>
          <div className="flex gap-2 p-1 bg-surface-dark rounded-2xl border border-dark overflow-x-auto w-full md:w-auto">
            {categories.map(cat => (
              <button
                key={cat}
      ...
      {/* Grid Game */}
      <section id="games-section" className="px-6 py-16 max-w-7xl mx-auto w-full">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">
          {filteredGames.map(game => (
            <div
              ...
              className="group relative glass-panel rounded-3xl overflow-hidden border border-dark hover:border-primary/50 transition-all hover:-translate-y-3 hover:shadow-neon cursor-pointer"
            >
              <div className="aspect-[3/4] relative">
                <div className="absolute top-4 left-4 z-20 flex flex-col gap-2">
                  {game.isHot && <span className="px-3 py-1 bg-secondary text-main text-[10px] font-bold rounded-full uppercase shadow-lg">Trending</span>}
              ...
              <div className="p-6 bg-surface-dark relative">
                ...
                <h3 className="font-bold text-xl text-main truncate group-hover:text-primary transition-colors">{game.name}</h3>

                {/* Mobile visible button indicator - Hidden as requested */}
      ...
      {showPromo && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-background-dark/90 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="glass-panel w-full max-w-lg rounded-3xl p-8 border border-secondary shadow-neon relative transform transition-all scale-100">
            <button onClick={() => setShowPromo(false)} className="absolute top-4 right-4 text-slate-400 hover:text-white transition-colors">
              <span className="material-symbols-outlined">close</span>
            </button>
            <div className="text-center mb-8">
              <div className="inline-flex justify-center items-center size-16 rounded-full bg-secondary/10 text-secondary mb-4 border border-secondary/20 shadow-lg">
                <span className="material-symbols-outlined text-3xl">local_offer</span>
              </div>
              <h2 className="text-2xl font-bold uppercase tracking-wide">Promo Spesial</h2>
              <p className="text-slate-400 text-sm mt-2">Salin kode voucher di bawah ini dan gunakan saat checkout!</p>
            </div>

            <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
              {activeVouchers.length > 0 ? activeVouchers.map(v => (
                <div key={v.id} className="bg-surface-dark border border-dashed border-slate-600 p-4 rounded-xl flex items-center justify-between group hover:border-secondary transition-colors">
                  <div>
                    <h3 className="font-bold text-xl text-main font-mono tracking-wider">{v.code}</h3>
                    <p className="text-xs text-secondary font-bold uppercase tracking-widest mt-1">Diskon {v.discountPercent}%</p>
                    {v.gameId && <p className="text-[10px] text-slate-500 mt-1">Khusus {games.find(g => g.id == v.gameId)?.name || 'Game Tertentu'}</p>}
                  </div>
                  <button
                    onClick={() => {
                    ...
                    className="bg-secondary/10 hover:bg-secondary text-secondary hover:text-white px-4 py-2 rounded-lg text-xs font-bold transition-all uppercase tracking-widest"
                  >
                    Salin
                  </button>
                </div>
                ...
                <div className="text-center py-8 text-slate-500 italic">
                  Belum ada promo aktif saat ini. Cek lagi nanti!
                </div>
              )}
            </div>
          </div>
